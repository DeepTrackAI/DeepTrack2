...
from deeptrack.backend import mie, TORCH_AVAILABLE
...
class TestMieTorch(unittest.TestCase):
    ...
    def test_coefficients_matches_numpy_and_autodiff(self):
        ...
        self.assertTrue(
            np.allclose(A.detach().numpy(), A_expected, rtol=1e-10, atol=1e-10)
        )
        self.assertTrue(
            np.allclose(B.detach().numpy(), B_expected, rtol=1e-10, atol=1e-10)
        )
