...
class TestRandom(unittest.TestCase):
    ...
    def test_permutation(self):
        ...
        # Tensor input (copy, not in-place)
        x = torch.arange(12).reshape(3, 4)
        original = x.clone()
        y = rnd.permutation(x)

        self.assertEqual(tuple(y.shape), (3, 4))
        self.assertTrue(torch.all(torch.sort(y[:, 0]).values ==
                                torch.sort(original[:, 0]).values).item())
