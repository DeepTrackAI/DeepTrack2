...
- `coerce(images)`: Ensures all inputs are converted to `Image` objects and
    coerced to the same type (e.g., NumPy or CuPy arrays).

- `strip(value)`: Recursively removes `Image` wrappers from nested structures.

- pad_image_to_fft(image: Union[Image, np.ndarray], axes: Iterable[int] = (0, 1)) -> Union[Image, np.ndarray]
...
from .types import NumberLike  # NumberLike = Union[np.ndarray, int, float, bool,
                            #                 complex, cupy.ndarray,
                            #                 torch.Tensor]
...
class Image:
    ...
    1. **Unified Interface**: Offers compatibility with NumPy and CuPy
        operations regardless of the underlying data type. This allows for
        seamless integration with NumPy functions and universal function calls
        (`ufuncs`).

    2. **Property Tracking**: Stores and manages properties associated with
        features used in creating or modifying the image. This makes it
        possible to track metadata and ensure consistency across operations.
    ...
    >>> import numpy as np
    >>> img = Image(np.array([1, 2, 3]))
    >>> img + 10  # Operations are supported
    Image(array([11, 12, 13]))

    >>> img.append({"property_name": "value"})  # Track properties
    >>> img.get_property("property_name")
    'value'
...
def pad_image_to_fft(
    ...
    so that their lengths match the nearest larger size in `FASTEST_SIZES`. 
    ...
    ------
    ValueError
        If no suitable size is found in `FASTEST_SIZES` for any axis length.
    ...
    def _closest(
        ...
        # Returns the smallest value frin FASTEST_SIZES larger than dim.
        for size in FASTEST_SIZES:
            if size >= dim:
                return size
        raise ValueError(
            f"No suitable size found in FASTEST_SIZES={FASTEST_SIZES} "
            f"for dimension {dim}."
        )
