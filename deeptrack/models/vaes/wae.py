...
class WAE(tf.keras.Model):
    ...
    def compile(
        ...
        if enc_optimizer is None:
            enc_optimizer = tf.keras.optimizers.Adam(learning_rate=1e-3, beta_1=0.5, beta_2=0.999)

        if dec_optimizer is None:
            dec_optimizer = tf.keras.optimizers.Adam(learning_rate=1e-3,  beta_1=0.5, beta_2=0.999)

        match self.regularizer:
            case "gan":
                if disc_optimizer is None:
                    disc_optimizer = tf.keras.optimizers.Adam(learning_rate=5e-4, beta_1=0.5, beta_2=0.999)
    ...
    def mmd_penalty(self, pz,qz, batch_size):
        ...
        cbase = tf.constant(2. * self.latent_dim * self.sigma_z)
        stat = tf.constant(0.)
