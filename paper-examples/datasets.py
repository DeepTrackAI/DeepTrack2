...
def load(id, force_overwrite=False):
    ...
    if id not in _ID:
        print("Dataset", id, "not recognized. Available datasets are:", ", ".join(_ID.keys()))
        return 

    tag, folder, _ = _ID[id]

    if not force_overwrite and os.path.exists(os.path.join(root, folder)):
        print(id, "already downloaded! Use force_overwrite=True to redownload the dataset.")
        return 
...
def load_model(id, force_overwrite=False):
    ...
    if not force_overwrite and os.path.exists(destination):
        print(id, "already downloaded! Use force_overwrite=True to redownload the model.")
        return destination
