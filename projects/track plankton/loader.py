...
def stationary_spherical_plankton(
    im_size_height=256, im_size_width=256, radius=0.3e-6, label=0
):
    plankton = Sphere(
        ...
        radius=lambda: (
            (radius) + np.random.rand() * 0.5 * radius
        ),  # Dimensions of the principal axes of the ellipsoid
...
def moving_spherical_plankton(
    im_size_height=256,
    im_size_width=256,
    radius=0.3e-6,
    label=0,
    diffusion_constant_coeff=1,
):
    plankton = Sphere(
        position_unit="pixel",  # Units of position (default meter)
        position=lambda: np.random.rand(2) * np.array([im_size_height, im_size_width]),
        z=lambda: -1.0 + np.random.rand() * 0.5,
        radius=lambda: (
            (radius) + np.random.rand() * 0.2 * radius
        ),  # Dimensions of the principal axes of the ellipsoid
        refractive_index=lambda: 0.9 + 1 * (0.1j + np.random.rand() * 0.00j),
        upsample=4,  # Amount the resolution is upsampled for accuracy
        particle_type=label,
        diffusion_constant=lambda: (1 + np.random.rand() * 9)
        * 2e-14
...
def stationary_ellipsoid_plankton(
    im_size_height=256, im_size_width=256, radius=(1.5e-7, 9e-7, 1.5e-7), label=0
):
    plankton = Ellipsoid(
        position_unit="pixel",  # Units of position (default meter)
        position=lambda: np.random.rand(2) * np.array([im_size_height, im_size_width]),
        z=lambda: -1.0 + np.random.rand() * 0.5,
        radius=lambda: (
            radius + np.random.rand(3) * (0.1) * radius
        ),  # Dimensions of the principal axes of the ellipsoid
...
def moving_ellipsoid_plankton(
    im_size_height=256,
    im_size_width=256,
    radius=(1.5e-7, 9e-7, 1.5e-7),
    label=0,
    diffusion_constant_coeff=1,
):
    plankton = Ellipsoid(
...
def get_position_moving_plankton(
    sequence_step,
    previous_value,
    diffusion_constant,
    alpha,
    ...
    R = np.matmul(Rz, Ry)
    position_circle = np.array(
        [
            0,
            helix_radius * np.sin(phi + phi_dot * sequence_step * dt),
...
def plankton_brightfield(im_size_height=256, im_size_width=256, gradient_amp=1):
    spectrum = np.linspace(400e-9, 700e-9, 3)

    illumination_gradient = IlluminationGradient(
        gradient=lambda: np.random.randn(2) * 0.0008 * gradient_amp
    )
...
def create_image(noise_amp, sample, microscope, norm_min, norm_max):
    noise = Poisson(
        snr=lambda: (60 + np.random.rand() * 30) * 1 / (max(0.01, noise_amp))
    )
    incoherently_illuminated_sample = sum(
        [
            brightfield_microscope_one_wavelegth(sample)
            for brightfield_microscope_one_wavelegth in microscope
        ]
    )
    ...
    image = (
        augmented_image
        + noise
        + NormalizeMinMax(min=norm_min, max=norm_max)
        + Clip(min=0, max=1)
    )
...
def get_target_sequence(sequence_of_particles):
    ...
    if no_of_types == 1:
        ...
        indices = np.asarray(sequence_of_particles).shape[0]
        label = np.zeros(
            (*np.asarray(sequence_of_particles).shape[1:3], no_of_types + 1)
        )
        X, Y = np.meshgrid(
            np.arange(0, np.asarray(sequence_of_particles).shape[2]),
            np.arange(0, np.asarray(sequence_of_particles).shape[1]),
...
def create_custom_batch_function(
