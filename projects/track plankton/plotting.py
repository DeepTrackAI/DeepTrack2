...
import cv2
from utils import (
    get_mean_net_and_gross_distance,
    extract_positions_from_list,
)
...
def plot_net_vs_gross_distance(list_of_plankton=None, **kwargs):
    net_distances, gross_distances = get_mean_net_and_gross_distance(
        list_of_plankton, **kwargs
    )
    plt.figure(figsize=(8, 8))
    plt.axis(
        [
            0,
            max(gross_distances[gross_distances != 0]) * 1.1,
            0,
            max(net_distances[net_distances != 0]) * 1.1,
        ]
    )
    X = gross_distances
    Y = net_distances
...
def plot_and_save_track(
    ...
    **kwargs
    ...
    for i, j in enumerate(range(frame_im0, frame_im0 + no_of_frames)):
        ...
        if show_plankton_track:
            ax.plot(
                scale_width * positions_track[max(i - 5, 0) : i + 1, 1::2],
                scale_height * positions_track[max(i - 5, 0) : i + 1, 0::2],
                c=color_plankton_track,
                linewidth=1,
            )
            ax.scatter(
                scale_width * positions_track[i, 1::2],
                scale_height * positions_track[i, 0::2],
                s=100,
                marker=".",
                facecolor="none",
                edgecolors=color_plankton_track,
            )

        if show_plankton_dont_track:
            ax.plot(
                scale_width * positions_dont_track[max(i - 5, 0) : i + 1, 1::2],
                scale_height * positions_dont_track[max(i - 5, 0) : i + 1, 0::2],
                c=color_plankton_dont_track,
                linewidth=1,
            )
            ax.scatter(
                scale_width * positions_dont_track[i, 1::2],
                scale_height * positions_dont_track[i, 0::2],
                s=100,
                marker=".",
                facecolor="none",
                edgecolors=color_plankton_dont_track,
            )
        ...
        if show_specific_plankton:
            for num in specific_plankton:
                ax.plot(
                    scale_width
                    * plankton_track["plankton%d" % num].positions[
                        max(i - 5, 0) : i + 1, 1
                    ],
                    scale_height
                    * plankton_track["plankton%d" % num].positions[
                    ...
                    c=color_specific_plankton,
                    linewidth=1,
                )
                ax.scatter(
                    scale_width * plankton_track["plankton%d" % num].positions[i, 1],
                    scale_height * plankton_track["plankton%d" % num].positions[i, 0],
                    s=100,
                    marker=".",
                    facecolor="none",
                    edgecolors=color_specific_plankton,
                )
        ...
        if show_numbers_track:
            for key in plankton_track:
                ax.annotate(
                    key.replace("plankton", ""),
                    (
                        scale_width * plankton_track[key].positions[i, 1] - 27,
                        scale_height * plankton_track[key].positions[i, 0] - 10,
                    ),
                    color=color_plankton_track,
                    fontsize=10,
                )
        ...
        if show_numbers_dont_track:
            for key in plankton_dont_track:
                ax.annotate(
                    key.replace("plankton", ""),
                    (
                        scale_width * plankton_dont_track[key].positions[i, 1] - 27,
                        scale_height * plankton_dont_track[key].positions[i, 0] - 10,
                    ),
                    color=color_plankton_dont_track,
                    fontsize=10,
                )
            ...
            for num in specific_plankton:
                ax.annotate(
                    ...
                    (
                        ...
                        - 27,
                        scale_height
                        * plankton_track["plankton%d" % num].positions[i, 0]
                        - 10,
                    ),
                    color=color_specific_plankton,
                    fontsize=10,
                )
        ...
        if save_images:
            plt.savefig(
                ...
                + "\\"
                + frame_name
                + "%0{}d".format(len(str(no_of_frames))) % j
                + file_type
            )
            plt.close(fig)
        else:
