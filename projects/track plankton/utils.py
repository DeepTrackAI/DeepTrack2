    ...
    **kwargs
    ...
    for i in range(len(list_paths)):
        if list_paths[i][-3:] != first_file_format:
            print(
                "Only the images to be analyzed can be in the folder with the images."
            )
            break
    ...
    start_point, end_point = max(center_frame - frames_one_dir, 0), min(
        center_frame + frames_one_dir + 1, len(list_paths)
    )
    for i in range(start_point, end_point):
        mean_image += (
            normalize_image(cv2.imread(path_folder + "\\" + list_paths[max(i, 0)], 0))
            / tot_no_of_frames
        )

    img = normalize_image(cv2.imread(path_folder + "\\" + list_paths[center_frame], 0))
    resized_center_image = cv2.resize(
        img, dsize=(im_width, im_height), interpolation=cv2.INTER_AREA
    )
    resized_mean_image = normalize_image(
        cv2.resize(
            mean_image, dsize=(im_width, im_height), interpolation=cv2.INTER_AREA
        )
    )
...
def get_mean_image(folder_path, im_size_width, im_size_height):
    ...
    resized_mean_image = cv2.resize(
        mean_img, dsize=(im_size_width, im_size_height), interpolation=cv2.INTER_AREA
    )
    ...
    **kwargs
    ...
    for count, num in enumerate(outputs):
        if type(num) == int:
            ...
            img = cv2.resize(
                img,
                dsize=(im_resize_width, im_resize_height),
        ...
        if type(num) == list:
            img0 = normalize_image(
                cv2.imread(folder_path + "\\" + list_paths[frame_im0 + num[0]], 0)
            )
            img1 = normalize_image(
                cv2.imread(folder_path + "\\" + list_paths[frame_im0 + num[1]], 0)
            )
            diff = img1 - img0
            diff = cv2.resize(
                diff,
...
def get_blob_center(label, array):
    row, col = np.where(array == label)
    if len(row) == 0:
        return np.nan, np.nan
    row_center = np.sum(row) / (len(row))
    col_center = np.sum(col) / (len(col))
...
class Plankton:
    def __init__(self, position, number_of_timesteps, current_timestep):
        self.positions = (
            np.zeros(shape=(number_of_timesteps, 2), dtype="object") * np.nan
        )
    ...
    def get_latest_position(self, timestep=0, time_threshold=10, **kwargs):
        latest_position = [np.nan, np.nan]
        for i in range(
            self.number_of_timesteps - timestep + 1,
            min(
                time_threshold + self.number_of_timesteps - timestep,
    ...
    def get_mean_velocity(self):
        ...
        no_of_numbers = np.count_nonzero(
            ~np.isnan(self.positions[:, 0].astype("float"))
        )
        mean_velocity = 0
        if no_of_numbers > 1:
            for i in range(no_of_timesteps - 1):
                mean_velocity += np.nansum(
                    np.linalg.norm(self.positions[i, :] - self.positions[i + 1, :])
                ) / (no_of_numbers)
            else:
                mean_velocity = 0
...
def initialize_plankton(
    positions=None, number_of_timesteps=None, current_timestep=0, **kwargs
):
    ...
    for i in range(no_of_plankton):
        list_of_plankton["plankton%d" % i] = Plankton(
            positions[i], number_of_timesteps, current_timestep
        )
    return list_of_plankton


def update_list_of_plankton(
    list_of_plankton=None,
    positions=None,
    max_dist=10,
    timestep=None,
    time_threshold=10,
    ...
    **kwargs
    ...
    if list_of_plankton is None:
        ...
        list_of_plankton = initialize_plankton(
            positions=positions,
            number_of_timesteps=no_of_timesteps,
    ...
    for value, key in enumerate(list_of_plankton):
        plankton_positions[value, :] = list_of_plankton[key].get_latest_position(
            timestep=timestep, time_threshold=time_threshold
        )

    distances0 = cdist(positions, plankton_positions)

    if extrapolate == True and timestep > 1:
        plankton_positions = extrapolate_positions(
    ...
    for i in range(no_of_positions):
        if np.nanmin(distances[i, :]) > max_dist:
            ...
            list_of_plankton["plankton%d" % len(list_of_plankton)] = Plankton(
                position,
                list_of_plankton["plankton%d" % 0].number_of_timesteps,
                timestep,
            )
        else:
            if np.sum(distances[i, :] < max_dist) > 1:
                ...
                for j in range(len(temp_dists)):
                    temp_veldiffs[j] = np.abs(
                        temp_dists[j]
                        - list_of_plankton[
                            "plankton%d" % temp_indices[j]
                        ].get_mean_velocity()
                    )
                temp_min_vel = np.min(temp_veldiffs)
                temp_min_index = np.where(temp_veldiffs == temp_min_vel)[0][0]
                list_of_plankton[
                    "plankton%d" % temp_indices[temp_min_index]
                ].add_position(positions[i, :], timestep)

            else:
                temp_min_dist = np.nanmin(distances[i, :])
                temp_min_index = np.nonzero(distances[i, :] == temp_min_dist)[0][0]
                list_of_plankton["plankton%d" % temp_min_index].add_position(
                    positions[i, :], timestep
                )
    return list_of_plankton
...
def assign_positions_to_planktons(positions, **kwargs):
    no_of_timesteps = len(positions)
    list_of_plankton = initialize_plankton(
        positions=positions[0], number_of_timesteps=no_of_timesteps
    )

    for i in range(1, no_of_timesteps):
        list_of_plankton = update_list_of_plankton(
            list_of_plankton=list_of_plankton,
...
def interpolate_gaps_in_plankton_positions(list_of_plankton=None, **kwargs):
    ...
    for key in list_of_plankton:
        for j in range(1, no_of_timesteps - 1):
            if (
                np.isnan(list_of_plankton[key].positions[j, 0])
                and np.any(
                    np.isnan(
                        [
                            list_of_plankton[key].positions[j - 1, 0],
                            list_of_plankton[key].positions[j + 1, 0],
                        ]
                    )
                )
                ...
                list_of_plankton[key].positions[j, 0] = (
                    list_of_plankton[key].positions[j - 1, 0]
                    + list_of_plankton[key].positions[j + 1, 0]
                ) / 2

                list_of_plankton[key].positions[j, 1] = (
                    list_of_plankton[key].positions[j - 1, 1]
                    + list_of_plankton[key].positions[j + 1, 1]
                ) / 2
    return list_of_plankton
...
def extrapolate_positions(list_of_plankton=None, timestep=2, **kwargs):
    ...
    for value, key in enumerate(list_of_plankton):
        if (
            np.isnan(list_of_plankton[key].positions[timestep, 0])
            and np.any(
                np.isnan(
                    [
                        list_of_plankton[key].positions[timestep - 1, 0],
            ...
            plankton_positions[value, 0] = (
                2 * list_of_plankton[key].positions[timestep - 1, 0]
                - list_of_plankton[key].positions[timestep - 2, 0]
            )

            plankton_positions[value, 1] = (
                2 * list_of_plankton[key].positions[timestep - 1, 1]
                - list_of_plankton[key].positions[timestep - 2, 1]
            )
...
def trim_list_from_stationary_planktons(
    list_of_plankton=None, min_distance=10, **kwargs
):
    ...
    for key in list(list_of_plankton):
        ...
        for i in range(len(temp_positions[:, 0]) - 1):
            temp_dist += np.nansum(
                np.linalg.norm(temp_positions[i, :] - temp_positions[i + 1, :])
            )
...
def split_plankton(list_of_plankton=None, percentage_threshold=0.5, **kwargs):
    no_of_timesteps = list_of_plankton[
        list(list_of_plankton.keys())[0]
    ].number_of_timesteps
    min_positions = int(percentage_threshold * no_of_timesteps)
    plankton_track = {}
    for key in list_of_plankton:
        number_non_nan = np.count_nonzero(
            ~np.isnan(list_of_plankton[key].positions[:, 0].astype("float"))
        )
    ...
    for i in range(no_of_plankton):
        ...
        for j in range(no_of_timesteps):
            if np.isnan(positions[j, i * 2]):
                ...
                break
        positions[:, 2 * i : 2 * (i + 1)] = np.roll(
            positions[:, 2 * i : 2 * (i + 1)], -counter, axis=0
        )
    ...
    for i in range(no_of_timesteps - 1):
        ...
        for j in range(no_of_plankton):
            if not np.isnan(positions[i + 1, 2 * j]):
                counter += 1
                mean_net_distances[i] += np.linalg.norm(
                    positions[i + 1, 2 * j : 2 * (j + 1)]
                    - positions[0, 2 * j : 2 * (j + 1)]
                )

                for k in range(0, i + 1):
                    if np.isnan(positions[k + 1, 2 * j]):
                        break
                    mean_gross_distances[i] += np.linalg.norm(
                        positions[k + 1, 2 * j : 2 * (j + 1)]
                        - positions[k, 2 * j : 2 * (j + 1)]
                    )
        if counter == 0:
...
def save_positions(
    list_of_plankton, save_path=None, file_format=".xlsx", pixel_length_ratio=1
):
    shape_position = np.shape(
        list_of_plankton[list(list_of_plankton.keys())[0]].positions.astype("float64")
    )
    ...
    for i, key in enumerate(list_of_plankton):
        positions_array[:, 2 * i : 2 * (i + 1)] = list_of_plankton[
            key
...
def fix_positions_from_cropping(
    positions, col_delete_list=[None], row_delete_list=[None], **kwargs
):
